...
if [ $# != 2 ]; then
  echo "extract_event_windowing_stats.sh basename(MEASURE) basecmp(0:ENZ, 1:RTZ)"; exit
fi
...
for win in ${basename}/*.win.qual ; do
  ...
  if [ $nwin -gt 0 ] ; then
    ...
    if [ $c == "Z" ] ; then
      ...
      echo ">" >> $t2n
      echo "$min_time $ddg 0.0" >> $t2n
      tail -$nwin $win | awk '{printf "%f %f %f\n%f %f %f\n%f %f %f\n%f %f %f\n", $2,d,0,$2,d,1,$3,d,1,$3,d,0}' d=$ddg >> $t2n
      echo "$t_end $ddg 0.0" >> $t2n
    elif [ $c == "E" ] ; then
      echo ">" >> $t2e
      echo "$min_time $ddg 0.0" >> $t2e
      tail -$nwin $win | awk '{printf "%f %f %f\n%f %f %f\n%f %f %f\n%f %f %f\n", $2,d,0,$2,d,1,$3,d,1,$3,d,0}' d=$ddg >> $t2e
      echo "$t_end $ddg 0.0" >> $t2e
    fi
...
proj=m0.8
area=119.0/123.5/21.4/26.0
echo $proj
psbasemap -J$proj -R$area -B2/2 -G$paleblue -K -P -Y5 -V > $out
pscoast -J$proj -R$area -Df -W3 -G$grey -O -K -V >> $out
psxy $t3 -J$proj -R$area -: -M -W3 -O -K -V >> $out
echo $evlo $evla | psxy -J$proj -R$area -Sa0.25 -G$red -O -K -V >> $out
...
awk '{print $6}' $t0 | pshistogram  -J$proj -F -B:"CC":/WS -W0.02 -Lthin -G$grey -O -K -X-5 -Y-3.2 >> $out
awk '{print $5}' $t0 | pshistogram  -J$proj -F -B:"@~t@~ / s":/WS -W1 -Lthin -G$grey -O -K -X2.7 >> $out
awk '{print $7}' $t0 | pshistogram  -J$proj -F -B:"@~D@~lnA":/WS -W0.1 -Lthin -G$grey -O -X2.7 >> $out
...
psbasemap -R$region -J$proj -B:"Vertical component":/EN -O -K >> $out2
...
psbasemap -R$region -J$proj -B:"Radial component":/EN -O -K >> $out2
psbasemap -R$region -J$proj -B${timestep}:"Time(s)":/${degstep}:"Distance / degree":WS -O -K -Y-3.5 >> $out2
pswiggle $t2t -R$region -J$proj -Z20 -M -P -W1/$black -G$blue -O -K >> $out2 
psbasemap -R$region -J$proj -B:"Transverse component":/EN -O >> $out2
...
psbasemap -R$region -J$proj -B:"Vertical component":/EN -O -K >> $out2
psbasemap -R$region -J$proj -B${timestep}:"Time(s)":/${degstep}:"Distance / degree":WS -O -K -Y-3.5 >> $out2
pswiggle $t2n -R$region -J$proj -Z20 -M -P -W1/$black -G$green -O -K >> $out2 
psbasemap -R$region -J$proj -B:"Radial component":/EN -O -K >> $out2
psbasemap -R$region -J$proj -B${timestep}:"Time(s)":/${degstep}:"Distance / degree":WS -O -K -Y-3.5 >> $out2
pswiggle $t2e -R$region -J$proj -Z20 -M -P -W1/$black -G$blue -O -K >> $out2 
psbasemap -R$region -J$proj -B:"Transverse component":/EN -O >> $out2
...
rm -f $t0 $t1 $t2z $t2r $t2t $t2n $t2e $t3
