...
#
# EXAMPLES:
#   prepare_adj_src.pl -m PLOTS/CMTSOLUTION_9818433 -z BH -s STATIONS_CI -o ADJOINT_SOURCES -i OUTPUT_FILES/*adj -r
...
#      $tcomp =~s/$cmpZ/$cmpT/; $ncomp=~s/$cmpZ/$cmpN/; $ecomp=~s/$cmpZ/$cmpE/;}
    ...
    if (@lines == 0) {
#       die("station not listed in $stafile\n");
        system("rm -f $opt_i/$sta*");
    }
    foreach $line (@lines) {
    ...
    if ($opt_i) {
      if ($opt_r) {
        ...
        if (defined $adj{$stanet}{$cmpE} or defined $adj{$stanet}{$cmpN} or defined $adj{$stanet}{$cmpZ}) {
          if (defined $adj{$stanet}{$cmpE} ) {
            $ecomp = $adj{$stanet}{$cmpE}; $ncomp = $ecomp; $zcomp = $ecomp;
            $zcomp=~s/$cmpE/$cmpZ/;$ncomp=~s/$cmpE/$cmpN/;}
          elsif (defined $adj{$stanet}{$cmpN}) {
            $ncomp = $adj{$stanet}{$cmpN}; $ecomp = $ncomp; $zcomp = $ncomp;
            $zcomp=~s/$cmpN/$cmpZ/;$ecomp=~s/$cmpN/$cmpE/;}
          else {
            $zcomp = $adj{$stanet}{$cmpZ}; $ecomp = $zcomp; $ncomp = $zcomp;
            $ncomp=~s/$cmpZ/$cmpN/;$ecomp=~s/$cmpZ/$cmpE/;}

          system("./check_adj_src $zcomp $ecomp $ncomp");
          if ($? != 0) {
    	    die("Error: check_adj_src $zcomp $ecomp $ncomp\n");
          }
          system("cp -f $ecomp $ncomp $zcomp $outdir");

        } else {die("check hash table for adj{$stanet}{$cmpE,$cmpN,$cmpZ}\n");}
      }
    }
