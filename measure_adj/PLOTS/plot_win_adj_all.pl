...
#   plot_win_adj_all.pl -l -10/200 -m ../CMTSOLUTION_9818433 -n BH -b 0 -k 7/1 -a STATIONS_ADJOINT -d DATA -s SYN -c RECON -w MEASUREMENT.WINDOWS -i m16 -j 6/30 -p
#
#----------------------------------------------------
...
sub Usage{
  ...
  plot_win_adj_all.pl -m CMTFILE -a STATION_FILE -n chan -b iboth -l tmin/tmax -k imeas/iadj -d data_dir -s syn_dir -c recon_dir -w winfile -i smodel -j Tmin/Tmax -p
...
if (@ARGV == 0) { Usage(); }
if (!getopts('m:a:n:b:d:k:l:s:c:w:i:j:p')) {die('Check input arguments\n');}
...
for($i = 0; $i < $ns; $i++){
  ...
  if ($opt_p) {
    print "\n ./plot_win_adj.pl -m $cmtfile -n $sta/$net/$chan -b $iboth -l $tcuts -k $opt_k -a $station_list -d $data_dir -s $syn_dir -c $recon_dir -w $winfile -i $smodel -j $Ts -p\n";
    `./plot_win_adj.pl -m $cmtfile -n $sta/$net/$chan -b $iboth -l $tcuts -k $opt_k -a $station_list -d $data_dir -s $syn_dir -c $recon_dir -w $winfile -i $smodel -j $Ts -p`;
  }
  else {
