...
sub Usage{
  ...
  with
       ...
       -p -- use R/T/Z components rather than E/N/Z
END
  exit(1);
}
...
if($ifault==1) {
    $dir0 = "~/Database/GMTPLOT";
    $fault_file = "$dir0/fault.dat";
#   $kcf_file   = "$dir0/faults/kcf.xy";
#   $breck_file = "$dir0/faults/breck.xy";
    $fault_infoK = "-MX -W0.5p,0/0/0";
...
$ntline=`grep -n \"$Tlab.semv\" $Winfile | awk -F: '{ print \$1 }'`;
chomp($ntline);
if ($ntline) {
  $nline=$ntline;
  $nline=$nline+1;
...
$nrline=`grep -n \"$Rlab.semv\" $Winfile | awk -F: '{ print \$1 }'`;
chomp($nrline);
#print "$nrline\n";
if ($nrline) {
  $nline=$nrline;
...
$nzline=`grep -n \"$Zlab.semv\" $Winfile | awk -F: '{ print \$1 }'`;
chomp($nzline);
if ($nzline) {
  $nline=$nzline;
  $nline=$nline+1;
...
if($iplot_adj == 1) {
  ...
  if ($opt_p) {
    ...
    $Zadj="ADJOINT_SOURCES/${sta}.${net}.${chan}Z.${klab}.adj";
    $Radj="ADJOINT_SOURCES/${sta}.${net}.${chan}N.${klab}.adj";
    $Tadj="ADJOINT_SOURCES/${sta}.${net}.${chan}E.${klab}.adj";}
  ...
  if ($iboth==1) {
    if ($opt_p) {
      ...
      $Zadj2="ADJOINT_SOURCES/$sta.$net.${chan}Z.${klab2}.adj";
      $Radj2="ADJOINT_SOURCES/$sta.$net.${chan}N.${klab2}.adj";
      $Tadj2="ADJOINT_SOURCES/$sta.$net.${chan}E.${klab2}.adj";}
  }
  ...
  if (-f $Radj) {
    ...
    if ($opt_p) {`echo \"$xtextpos1 $ytextpos2 14 0 1 BC R\" | pstext $proj $bounds -N -K -O >> $ps_file`;}
    else {`echo \"$xtextpos1 $ytextpos2 14 0 1 BC N\" | pstext $proj $bounds -N -K -O >> $ps_file`;}
    if ($iboth==1) {
  ...
  if (-f $Tadj) {
    ...
    if ($opt_p) {`echo \"$xtextpos1 $ytextpos2 14 0 1 BC T\" | pstext $proj $bounds -N -K -O >> $ps_file`;}
    else {`echo \"$xtextpos1 $ytextpos2 14 0 1 BC E\" | pstext $proj $bounds -N -K -O >> $ps_file`;}
