...
def plot_vertical_slices_vpvs(input_dir, output_dir):
    ...
    scalar_range = config_vert['fine_tune_vpvs']['scalar_range']
    cmap = config_vert['fine_tune_vpvs']['cmap']
    reverse_cmap = config_vert['fine_tune_vpvs']['reverse_cmap']
    plot_contour = config_vert['fine_tune_vpvs']['contour']['plot_contour']
    line_interval = config_vert['fine_tune_vpvs']['contour']['line_interval']
    annotation_interval = config_vert['fine_tune_vpvs']['contour']['annotation_interval']
    ...
    for profile_index, profile_info in profile_info_df.iterrows():
        ...
        # ---------------------------------------------------------------------------
        profile_range = [len_profile[0], len_profile[1], dep_range[0], dep_range[1]]
        selected_df = xyz_df[['lon','lat','dep', 'vpvs']]
        ...
        fig.colorbar(frame=[f'a0.1f0.1', f'x+lVp/Vs ratio'], position='JMR+o-35c/-4c+w10c/0.5c+ml', cmap=True)
        ...
        output_dir_for_fig = os.path.join(output_dir, 'fig', 'vertical', 'vpvs')
        check_if_output_dir_exist(output_dir=output_dir_for_fig)
        fig.savefig(f'{output_dir_for_fig}/profile_VpVs_{int(azi_profile)}_{letter_index}.png', dpi=300,
                    transparent=True)
