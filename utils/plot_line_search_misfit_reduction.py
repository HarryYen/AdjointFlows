...
def _line_search_measure_dir(tomo_dir, model_num, dataset_name, step_index):
    line_search_dir = f"{tomo_dir}/mtest{model_num:03d}"
    if dataset_name:
        return f"{line_search_dir}/MEASURE{step_index}_{dataset_name}"
    return f"{line_search_dir}/MEASURE{step_index}"


def get_chi(evlst, model_num, line_search_num, dataset_name=None):
    global tomo_dir

    if line_search_num == 0:
        measure_dir = f"{tomo_dir}/m{model_num:03d}/{_measure_dir_name(dataset_name)}/adjoints"
    else:
        measure_dir = _line_search_measure_dir(tomo_dir, model_num, dataset_name, line_search_num)
...
def get_misfit_list(model_num, ls_beg, ls_end, dataset_entries, base_dir, default_evlst_name=None):
    misfit_list = []
    for ls_num in range(ls_beg, ls_end+1):
        ...
        for dataset_entry in dataset_entries:
            ...
            misfit = get_chi(evlst, model_num, ls_num, dataset_name=dataset_name)
...
if __name__ == '__main__':
    ...
    # -----------------PARAMETERS----------------- #
    base_dir = '/home/harry/Work/AdjointFlows'
    ...
    misfit_list = get_misfit_list(
        model_num=model_num,
        ls_beg=line_search_n_list[0],
        ls_end=line_search_n_list[1],
