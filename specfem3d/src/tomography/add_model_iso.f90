...
!    - lbfgs_flag: <0 or 1> 0 for sd and 1 for lbfgs
! setup:
!
...
program add_model
  ...
  logical, parameter :: MINMAX_THRESHOLD_NEW = .true.
  ! threshold values
  ! minmax wavespeed values for southern california simulations
  real(kind=CUSTOM_REAL),parameter :: VS_MIN = 1500.0
  real(kind=CUSTOM_REAL),parameter :: VS_MAX = 5500.0
  real(kind=CUSTOM_REAL),parameter :: VP_MIN = 2600.0
  real(kind=CUSTOM_REAL),parameter :: VP_MAX = 9500.0
  ...
  if (lbfgs_flag == 0) then
    ! steepest descent method
    call get_direction_without_calculation_sd()
  else
    ! L-BFGS method
    call get_direction_without_calculation_lbfgs()
  endif
