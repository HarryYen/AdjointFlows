...
! reads in parameters needed (only step length for now...)
  ...
  character(len=MAX_STRING_LEN) :: s_lbfgs_flag
  ...
  call get_command_argument(2,s_lbfgs_flag)

  if (trim(s_lbfgs_flag) == '') then
    call tomo_usage()
  endif

  ! read in parameter information
  read(s_lbfgs_flag,*,iostat=ier) lbfgs_flag

  ! Check for conversion errors and if the value is 0 or 1
  if (ier /= 0 .or. (lbfgs_flag /= 0 .and. lbfgs_flag /= 1)) then
    print *, "Error: Argument must be an integer 0 or 1."
    call tomo_usage()
  end if
...
!
  ...
  if (myrank == 0) then
    print *,'Usage: add_model step_factor lbfgs_flag(0/1) [INPUT-KERNELS-DIR/] [OUTPUT-MODEL-DIR/]'
    print *
    print *,'with'
    print *,'  step_factor        - factor to scale gradient (e.g. 0.03 for 3 percent update)'
    print *,'  lbfgs_flag         - flag for telling sd or lbfgs (different updating way)'
    ...
    print *
    print *,'Please rerun e.g. like: mpirun -np ',sizeprocs,' ./bin/xadd_model 0.03 1'
    print *
  endif
